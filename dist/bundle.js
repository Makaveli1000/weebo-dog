...
import {
  ...
  increment,
  getDocs,
  setDoc // NEW: for setting specific document IDs (like call offers/answers)
...
// ============================================================================
const storage = getStorage(); // Initialize Firebase Storage globally once
...
// Function to handle cheering a player
...
// Attach event listeners for the "Cheer" buttons dynamically
...
onAuthStateChanged(auth, async (user) => {
  ...
  const videoCallPanel = document.getElementById('video-call-panel'); // NEW: Video Call Panel
  ...
  if (!user) {
    ...
    if (videoCallPanel) videoCallPanel.classList.add('hidden'); // NEW: Hide video call panel
    if (unsubscribePlayers) {
      unsubscribePlayers(); // Unsubscribe player listener on logout
      unsubscribePlayers = null;
    }
    resetCallState(); // NEW: Reset call state on logout
    if (incomingCallListenerUnsubscribe) incomingCallListenerUnsubscribe(); // NEW: Unsubscribe incoming call listener
  ...
  oracleSpeak(user, "The threads of fate align.");
  ...
  try {
    ...
    // NEW: Video Call setup for authenticated users
    if (videoCallPanel) videoCallPanel.classList.remove('hidden'); // Show video call panel
    listenForIncomingCalls(); // Start listening for incoming calls
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (loginForm) {
    loginForm.addEventListener("submit", (e) => {
      ...
      if (typeof window.logIn === 'function') {
        try { window.logIn(); } catch (err) { alert('Login failed: ' + err.message); }
      } else {
        alert("Login function not found. Ensure it's correctly exposed or imported.");
        console.error("window.logIn function is missing.");
      }
    });
  }
  ...
  // NEW: Event listeners for video calling buttons
